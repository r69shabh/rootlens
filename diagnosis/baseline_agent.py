...
from diagnosis.agent import CONFIDENCE_THRESHOLD, DiagnosisResult, _finalize, prepare_run
from diagnosis.anomaly_scan import AnomalousSegment
...
def _label_for_segment(
    tools: DiagnosisTools, seg: AnomalousSegment, current_start, current_end
) -> tuple[str, list[str]]:
    ...
    if dim == "issuer_bank":
        ...
        matrix = tools.compare_segments("issuer_bank", "card_network", current_start, current_end)
        ...
        checks.append(
            ...
            + (
                "also degraded -> would indicate network issue"
                if diffuse
                else "healthy across all networks -> isolated to this bank"
            )
        )
        return (
            f"bank_outage:{value}" if not diffuse else f"network_degradation:degraded_on_{value}"
        ), checks

    if dim == "card_network":
        matrix = tools.compare_segments("card_network", "issuer_bank", current_start, current_end)
        ...
        checks.append(
            ...
            + (
                ...
                if spans_issuers
                else "concentrated in one issuer -> bank outage"
            )
        )
    ...
    if dim == "merchant_id":
        ...
        checks.append(
            ...
            + (
                ...
                if pending
                else "failed volume -> merchant-side issue"
            )
        )
...
def rule_based_diagnose(
    ...
    baseline_start,
    baseline_end,
    scenario_id: str = "rule-baseline",
) -> DiagnosisResult:
    """Full deterministic pipeline: scan -> label -> evidence-backed verdict."""
    t0 = time.perf_counter()
    store, tools, segments, _onset = prepare_run(
        con,
        current_start,
        current_end,
        baseline_start,
    ...
    if not segments:
        ...
        if mix:
            ...
            if top_code == "gateway_timeout" and top_n >= 30 and top_n >= 2 * second_n:
                ...
                checks.append(
                    f"failure-code mix {mix}: gateway_timeout dominates and "
                ...
                checks.append(
                    f"failure-code mix {mix}: checkout_error dominates and "
        ...
        for s in segments[1:3]:
            if top.dimension == "issuer_bank" and s.dimension == "amount_bucket" and s.drop >= 0.3:
    ...
    result.transcript = [
        {"role": "context", "content": f"rule-based baseline, {len(segments)} segments"}
    ]
    result.store = store
    return result

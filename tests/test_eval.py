...
def _gt(tier="clean", labels=("bank_outage:ICICI",), types=("bank_outage",), sid="s1"):
    return {"scenario_id": sid, "difficulty_tier": tier, "expected_labels": list(labels),
...
def test_compound_requires_all_fault_types():
    gt = _gt(tier="compound", labels=("bank_outage:ICICI", "rule_trigger:10000"),
             types=("bank_outage", "high_ticket_rule"))
    full = DiagnosisResult(status="verdict",
                           root_cause="compound:bank_outage+rule_trigger", confidence=0.8)
